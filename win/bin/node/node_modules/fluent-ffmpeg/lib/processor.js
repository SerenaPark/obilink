...
exports = module.exports = function Processor(command) {
  ...
  this.takeScreenshots = function(config, folder, callback) {
    ...
    function _renderOutputName(j, offset) {
      ...
      //yoho:bugfixed - result = result.replace('%f', self.options.inputfile);
      //yoho:bugfixed - result = result.replace('%b', self.options.inputfile.substr(0,self.options.inputfile.lastIndexOf('.')));
      if (os.match(/win(32|64)/)) {
        result = result.replace('%f', self.options.inputfile.substring(self.options.inputfile.lastIndexOf('\\')+1));
        result = result.replace('%b', self.options.inputfile.substring(self.options.inputfile.lastIndexOf('\\')+1, self.options.inputfile.lastIndexOf('.')));
      }
      else {
        result = result.replace('%f', self.options.inputfile.substring(self.options.inputfile.lastIndexOf('/')+1));
        result = result.replace('%b', self.options.inputfile.substring(self.options.inputfile.lastIndexOf('/')+1, self.options.inputfile.lastIndexOf('.')));
  ...
  this.buildFfmpegArgs = function(overrideOutputCheck, meta) {
    ...
    if (this.options.video.skip) {
      ...
      if (this.options.video.bitrate) {
        //yoho:bugfixed - args.push('-b', this.options.video.bitrate + 'k');
